...
-- Description: Each branch's purchase detail and summary with ARYCoin, MG, SaveGold
-- =============================================

DECLARE @FromDate DATE = '2026-03-01'
DECLARE @ToDate DATE = '2026-03-09'
DECLARE @BranchName VARCHAR(100) = NULL  -- NULL = All Branches
...
WHERE LOWER(SourceType) = 'pur'
    AND (@BranchName IS NULL OR pur.BranchName LIKE '%' + @BranchName + '%')
GROUP BY pur.VenderName, pur.BranchName
ORDER BY TotalPurchaseAmount DESC

-- BRANCH WISE DAILY BREAKDOWN
SELECT pur.BranchName, CAST(pur.InsertedDateTime AS DATE) AS PurchaseDate,
    COUNT(*) AS TotalTransactions, SUM(pur.Amount) AS TotalAmount,
     SUM(pur.Amount * NULLIF(pur.ARYMargin/100,0)) as Profit
FROM #tbl2 pur WHERE LOWER(SourceType) = 'pur'
GROUP BY pur.BranchName, CAST(pur.InsertedDateTime AS DATE)
ORDER BY pur.BranchName, PurchaseDate

-- BRANCH DETAIL REPORT
SELECT  
    pur.BranchName, pur.VenderName, pur.CardStatus, pur.ProfileID_FK AS CustomerID, pur.Name AS CustomerName, pur.City,
    pur.SahulatMilestonePlan, pur.SahulatMileStonePercentage,
    pur.Amount, pur.GoldRate, pur.InsertedDateTime AS PurchaseDateTime, pur.TransactionID, pur.ProductName,
...
WHERE LOWER(SourceType) = 'pur'
    AND (@BranchName IS NULL OR pur.BranchName LIKE '%' + @BranchName + '%')
ORDER BY pur.BranchName, pur.InsertedDateTime DESC
