...
-- Description: Month wise purchase summary and detail with ARYCoin, MG, SaveGold
-- =============================================

DECLARE @Month INT = MONTH(GETDATE())  -- Current Month
DECLARE @Year INT = YEAR(GETDATE())    -- Current Year
...
-- ARYCoin (ValueBack)
SELECT t.ProfileID_FK, t.TransactionID, SUM(t.Amount) AS ARYCoin
INTO #Coin FROM #tbl2 t WHERE LOWER(SourceType) = 'vb'
GROUP BY t.ProfileID_FK, t.TransactionID

-- Mili Gold (MG)
SELECT t.ProfileID_FK, t.TransactionID, SUM(t.Amount) AS MG, AVG(t.GoldRate) AS GoldRate
INTO #MG FROM #tbl2 t WHERE LOWER(SourceType) = 'mg'
GROUP BY t.ProfileID_FK, t.TransactionID

-- SaveGold from MGPP
SELECT t.ProfileID_FK, t.TransactionID, SUM(t.CreatedOnAMountOf) AS CreatedOnAMountOf
INTO #MGSS FROM #MGPP t
GROUP BY t.ProfileID_FK, t.TransactionID
...
-- MONTHLY SUMMARY
SELECT  
    CAST(pur.InsertedDateTime AS DATE) AS PurchaseDate,
...
LEFT JOIN #MGSS ss ON ss.ProfileID_FK = pur.ProfileID_FK AND ss.TransactionID = pur.TransactionID
WHERE LOWER(SourceType) = 'pur'
GROUP BY CAST(pur.InsertedDateTime AS DATE),SahulatMilestonePlan
...
LEFT JOIN #MGSS ss ON ss.ProfileID_FK = pur.ProfileID_FK AND ss.TransactionID = pur.TransactionID
WHERE LOWER(SourceType) = 'pur'
GROUP BY CAST(pur.InsertedDateTime AS DATE)
ORDER BY PurchaseDate
