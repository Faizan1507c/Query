...
-- Description: Each vendor's purchase detail and summary with ARYCoin, MG, SaveGold
-- =============================================

DECLARE @FromDate DATE = '2026-03-01'
DECLARE @ToDate DATE = '2026-03-09'
DECLARE @VenderName VARCHAR(100) = NULL  -- NULL = All Vendors, or specify vendor name
...
SELECT  
    ...
    MIN(pur.InsertedDateTime) AS FirstTransaction,
    MAX(pur.InsertedDateTime) AS LastTransaction
FROM #tbl2 pur
...
WHERE LOWER(SourceType) = 'pur'
    AND (@VenderName IS NULL OR pur.VenderName LIKE '%' + @VenderName + '%')
GROUP BY pur.VenderName
ORDER BY TotalPurchaseAmount DESC

-- VENDOR WISE DETAIL REPORT
SELECT  
    pur.VenderName, pur.CardStatus, pur.ProfileID_FK AS CustomerID, pur.Name AS CustomerName, pur.City,
    pur.SahulatMilestonePlan, pur.SahulatMileStonePercentage,
    pur.Amount, pur.GoldRate, pur.InsertedDateTime AS PurchaseDateTime, pur.TransactionID, pur.BranchName, pur.ProductName,
...
WHERE LOWER(SourceType) = 'pur'
    AND (@VenderName IS NULL OR pur.VenderName LIKE '%' + @VenderName + '%')
ORDER BY pur.VenderName, pur.InsertedDateTime DESC

-- ONLINE VENDORS ONLY
SELECT pur.VenderName, COUNT(*) AS TotalTransactions, SUM(pur.Amount) AS TotalAmount,
...
LEFT JOIN #MGSS ss ON ss.ProfileID_FK = pur.ProfileID_FK AND ss.TransactionID = pur.TransactionID
WHERE LOWER(SourceType) = 'pur'
GROUP BY pur.VenderName ORDER BY TotalAmount DESC
